...
<main class="dashboard-main">
  @include('partials.paciente-header', ['title' => 'Mi Diario Personal', 'subtitle' => 'Registra tus emociones y conserva un historial privado de tu proceso.'])
  @include('partials.frontend-alerts')

  <section class="dashboard-section-grid diary-page-grid">
    <form method="POST" action="{{ route('paciente.diario.store') }}" class="dash-card diary-card">
      @csrf
      <div class="card-top">
        <div>
          <h2>Nueva entrada</h2>
      ...
      <label class="backend-field">Autorizar visualización del diario
        <select name="authorized_professional_id">
          ...
          @foreach(($diaryProfessionals ?? collect()) as $professional)
            <option value="{{ $professional->id }}">{{ $professional->nombre_completo }} · {{ ucfirst($professional->rol) }}</option>
          @endforeach
        </select>
      </label>
      ...
      <div class="emoji-selector">
        ...
        <div class="emoji-options">
          @foreach(['🙂','😌','😟','😢','😡','💪'] as $emoji)
            <label class="emoji-option"><input type="radio" name="emoji" value="{{ $emoji }}"><span>{{ $emoji }}</span></label>
          @endforeach
        </div>
      </div>
    ...
    <section class="dash-card diary-history-card">
      <div class="card-top diary-card-header-actions">
        ...
        <a class="btn-outline btn-sm" href="{{ route('paciente.diario.todas') }}">Ver todas las entradas</a>
      </div>
      ...
      <div class="diary-entries" id="diary-entries">
        @forelse($diaryEntries as $entry)
          <article class="diary-entry" data-index="{{ $loop->index }}" data-search="{{ e(($entry['title'] ?? '').' '.($entry['mood'] ?? '').' '.($entry['content'] ?? '').' '.($entry['date'] ?? '')) }}">
