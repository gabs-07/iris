...
<head>
  ...
  <link rel="stylesheet" href="{{ asset('css/gestion-citas.css') }}">
...
<main class="dashboard-main">
  @include('partials.paciente-header', ['title'=>'Mis citas y sesiones', 'subtitle'=>'Consulta tus próximas sesiones, solicitudes y el historial de atención.'])
...
<section class="appointment-summary patient-citas-summary">
  <article class="metric-card"><span>Próxima cita</span><strong>{{ $nextAppointment?->starts_at?->format('d') ?? '—' }}</strong><small>{{ $nextAppointment?->starts_at?->format('d/m/Y H:i') ?? 'Sin cita confirmada' }}</small></article>
  <article class="metric-card"><span>Citas activas</span><strong>{{ $upcoming->count() }}</strong><small>Sesiones confirmadas</small></article>
  <article class="metric-card"><span>Solicitudes</span><strong>{{ $requests->count() }}</strong><small>Pago, revisión o reagenda</small></article>
</section>
...
<section class="patient-tabs-toolbar appointment-tabs-native">
  <div class="tabs-container patient-appointment-tabs" role="tablist" aria-label="Secciones de citas">
    @foreach($tabs as $key => $tab)
      <button class="tab-btn {{ $loop->first ? 'active' : '' }}" type="button" data-tab="{{ $key }}" aria-selected="{{ $loop->first ? 'true' : 'false' }}">{{ $tab['label'] }}</button>
    @endforeach
  </div>
  <a class="new-appointment-floating" href="{{ route('paciente.buscar-especialista') }}" title="Nueva cita" aria-label="Nueva cita">+</a>
</section>

@foreach($tabs as $key => $tab)
  <section class="tab-content appointment-tab-content {{ $loop->first ? 'active' : '' }}" id="tab-{{ $key }}" data-tab-panel="{{ $key }}">
    <div class="appointments-list appointment-card-row patient-appointments-native {{ $key }}-tab">
      @forelse($tab['items'] as $appointment)
        @php
        ...
        @endphp
        <article class="appt-card-compact patient-appt-card patient-appt-native {{ $statusClass }} {{ $key === 'solicitudes' ? 'request-row' : '' }}" data-appointment-id="{{ $appointment->id }}">
          ...
          <div class="appt-details">
            <div class="appt-header-inline">
              <span class="status-badge {{ $statusClass }}">{{ $statusLabels[$effectiveStatus] ?? ucfirst($effectiveStatus) }}</span>
              <span class="time">{{ $timeLabel }} hrs</span>
              <span class="folio">{{ $displayFolio }}</span>
            </div>

            <div class="doc-info-compact patient-appointment-copy">
              <div>
                <h3>{{ $key === 'solicitudes' ? $requestTitle : $professionalName }}</h3>
                @if($key === 'solicitudes')
                  <span>{{ $professionalName }} · {{ $reason }} · {{ $modality }}</span>
                  <p class="request-meta">{{ $requestDescription }}</p>
                @else
                  <span>{{ $modality }} · {{ $reason }}</span>
                  @if($specialty)<small>{{ $specialty }}</small>@endif
                @endif
              </div>
            </div>

            @if($appointment->status === 'rescheduled')
              <div class="reschedule-note"><strong>Propuesta:</strong> {{ optional($appointment->reschedule_date)->format('d/m/Y') }} {{ $appointment->reschedule_time }} @if($appointment->reschedule_proposal) · {{ $appointment->reschedule_proposal }} @endif</div>
            @endif

            <div class="appointment-mini-meta">
              <span><strong>Pago:</strong> {{ $paymentLabels[$appointment->payment_status] ?? ucfirst($appointment->payment_status) }}</span>
              <span><strong>Total:</strong> ${{ number_format((float) $appointment->amount, 2) }} MXN</span>
            </div>
          </div>

          <div class="appt-actions">
            @if($key === 'proximas')
              @if($isSessionMoment && $videoLink)
                @php $hasPrimaryAction = true; @endphp
                <a class="btn-primary btn-sm" href="{{ route('paciente.sesion', ['appointment'=>$appointment->id]) }}">Entrar a sesión</a>
              @else
                <span class="session-muted">{{ $startsAt && $startsAt->lte($now) ? 'Disponible hasta '.optional($sessionAccessEnds)->format('H:i').' hrs' : 'Disponible 2 min antes' }}</span>
              @endif
              <div class="action-links">
                <button class="btn-link" type="button" data-open-reschedule data-appointment="{{ $appointment->id }}" data-specialist="{{ $professionalName }}" data-current="{{ $startsAt?->format('d/m/Y H:i') ?? 'Por definir' }}">Reagendar</button>
                <form class="inline-form" method="POST" action="{{ route('paciente.gestion-citas.cancelar',$appointment) }}">@csrf<input type="hidden" name="cancel_reason" value="Cancelada desde panel del paciente"><button class="btn-link danger" type="submit" onclick="return confirm('¿Cancelar esta cita?')">Cancelar cita</button></form>
              </div>
            @elseif($key === 'solicitudes')
              <div class="request-actions-bottom">
                @if($appointment->status === 'pending_payment')
                  <a class="btn-primary btn-sm" href="{{ route('paciente.pago-cita', ['appointment'=>$appointment->id]) }}">Pagar</a>
                @endif
                @if($appointment->status === 'rescheduled')
                  <form class="inline-form" method="POST" action="{{ route('paciente.gestion-citas.aceptar-reagenda',$appointment) }}">@csrf<button class="btn-primary btn-sm" type="submit">Aceptar horario</button></form>
                @endif
                @if($appointment->status === 'pending' && $appointment->requested_by === 'profesional')
                  <form class="inline-form" method="POST" action="{{ route('paciente.gestion-citas.aceptar-solicitud',$appointment) }}">@csrf<button class="btn-primary btn-sm" type="submit">Aceptar cita</button></form>
                @endif
                @if($appointment->status !== 'pending_payment')
                  <button class="btn-outline btn-sm" type="button" data-open-reschedule data-appointment="{{ $appointment->id }}" data-specialist="{{ $professionalName }}" data-current="{{ $startsAt?->format('d/m/Y H:i') ?? 'Por definir' }}">Reagendar</button>
                @endif
                @if(!in_array($effectiveStatus, ['completed','cancelled','rejected','missed'], true))
                  <form class="inline-form" method="POST" action="{{ route('paciente.gestion-citas.cancelar',$appointment) }}">@csrf<input type="hidden" name="cancel_reason" value="Cancelada desde panel del paciente"><button class="btn-link danger" type="submit" onclick="return confirm('¿Cancelar esta cita?')">Cancelar cita</button></form>
                @endif
              </div>
            @endif
  ...
  <section class="iris-action-modal" role="dialog" aria-modal="true" aria-labelledby="patient-reschedule-title">
    ...
    <form method="POST" id="patient-reschedule-form" class="modal-form-grid">
      ...
      <div class="form-row two">
        <label>Nueva fecha<input type="date" name="reschedule_date" required min="{{ today()->toDateString() }}"></label>
